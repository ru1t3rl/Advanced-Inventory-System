...
namespace Ru1t3rl.Events
{
    public class EventManager : UnitySingleton<EventManager>
    {
        ...
        [SerializeField] private List<CustomEvent<System.EventArgs>> paramEvents = new List<CustomEvent<System.EventArgs>>();
        ...
        public void AddListener(string eventName, System.Action<System.EventArgs> listener)
        {
            ...
            {
                paramEvents.Add(new CustomEvent<System.EventArgs>(eventName));
            }
        ...
        public void AddEvent(string eventName, UnityEvent<System.EventArgs> unityEvent)
        {
            ...
            {
                paramEvents.Add(new CustomEvent<System.EventArgs>(eventName));
                paramEvents[paramEvents.Count - 1].unityEvent = unityEvent;
            }
            else
            {
        ...
        /// <summary>Remove an event if it exists</summary>
        /// <param name="customEvent">The custom event to remove from the manager</param>
        public void RemoveEvent(CustomEvent<System.EventArgs> customEvent)
        {
            if (paramEvents.Contains(customEvent))
            {
                paramEvents.Remove(customEvent);
            }
        }
        ...
        /// <summary>Remove an event if it exists</summary> 
        /// <param name="unityEvent">The unity event to remove from the manager</param>
        public void RemoveEvent(UnityEvent<System.EventArgs> unityEvent)
        {
            if (paramEvents.ContainsEvent(unityEvent))
            {
                paramEvents.Remove(paramEvents.GetEvent(unityEvent));
            }
        }
